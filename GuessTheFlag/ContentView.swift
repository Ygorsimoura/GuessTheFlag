...
struct ContentView: View {
    @State private var scaleFlag = 1.0
    @State private var tapedflag = -1
    @State private var opacitie = 1.0
    @State private var rotationStates = [0.0, 0.0, 0.0]
                    ...
                    ForEach(0..<3) { number in
                        Button{
                            ...
                            Image(contries[number])
                                ...
                                .rotation3DEffect(.degrees(rotationStates[number]), axis: (x: 0, y: 1, z: 0))
                                .opacity(tapedflag == number ? 1 : opacitie)
                                .scaleEffect(tapedflag == number ? 1 : scaleFlag)
                        }
                    }
                }
